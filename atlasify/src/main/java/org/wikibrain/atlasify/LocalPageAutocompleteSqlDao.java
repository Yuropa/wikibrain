...
public class LocalPageAutocompleteSqlDao extends AbstractSqlDao<LocalPage> implements LocalPageDao {
    ...
    public List<LocalPage> getBySimilarTitle(Title title, NameSpace nameSpace, LocalLinkDao linkDao) throws DaoException {
        DSLContext context = getJooq();
        try {
            List<Result<Record>> beginsResults = context.select().
                    from(Tables.LOCAL_PAGE).
            ...
            Result<Record> beginRecords = beginsResults.get(0);
            Iterator<Record> beginIt = beginRecords.iterator();

            LinkedHashMap<Number, LocalPage> sortedRecords = new LinkedHashMap<Number, LocalPage>();

            while (beginIt.hasNext()) {
                Record r = beginIt.next();
            ...
            List<Result<Record>> containsRestuls = context.select().
                    from(Tables.LOCAL_PAGE).
                    where(Tables.LOCAL_PAGE.TITLE.likeIgnoreCase(title.getCanonicalTitle())).
                    and(Tables.LOCAL_PAGE.LANG_ID.eq(title.getLanguage().getId())).
                    and(Tables.LOCAL_PAGE.NAME_SPACE.eq(NameSpace.ARTICLE.getArbitraryId())).
                    limit(40).
                    fetchMany();

            Result<Record> containRecords = containsRestuls.get(0);
            Iterator<Record> containIt = containRecords.iterator();

            while (containIt.hasNext()) {
                Record r = containIt.next();
                LocalPage page = buildLocalPage(r);
                Iterator<LocalLink> links = linkDao.getLinks(page.getLanguage(), page.getLocalId(), false).iterator();
                int count = 0;
                while (links.hasNext()) {
